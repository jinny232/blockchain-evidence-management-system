...
import JudgeEvidenceDetailsModal from "@/components/judge/evidence/JudgeEvidenceDetailsModal";

const PAGE_SIZE = 10;

export default function JudgeEvidenceClient() {
  const [evidence, setEvidence] = useState<JudgeEvidenceItem[]>([]);
  const [selectedEvidence, setSelectedEvidence] =
    useState<JudgeEvidenceDetails | null>(null);
  ...
  async function loadEvidence() {
    ...
    try {
      const res = await fetch("/api/judge/evidence", {
        cache: "no-store",
      });
      ...
      if (!res.ok) {
        setEvidence([]);
        setError(data.message || "Failed to load judge evidence.");
      } else {
  ...
  async function openDetails(id: number) {
    ...
    try {
      const res = await fetch(`/api/judge/evidence/${id}`, {
        cache: "no-store",
      });
                ...
                >
                  ...
                  <td className="p-4 text-right">
                    <button
                      ...
                      className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700 hover:bg-amber-100 disabled:opacity-60"
                    >
