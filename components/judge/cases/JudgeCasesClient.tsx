...
import type { JudgeCaseItem } from "@/models/judge-case.model";

const PAGE_SIZE = 10;

export default function JudgeCasesClient() {
  ...
  async function loadCases() {
    ...
    try {
      ...
      if (!res.ok) {
        setCases([]);
        setError(data.message || "Failed to load judge cases.");
      } else {
      ...
      <section className="relative overflow-hidden rounded-3xl border border-amber-100 bg-gradient-to-br from-slate-950 via-amber-900 to-amber-500 p-7 text-white shadow-xl shadow-amber-200/70">
        ...
        <div className="relative">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-amber-50">
            Review assigned cases, evidence readiness, legal notes, and verdict
            status.
          </p>
        </div>
      </section>
