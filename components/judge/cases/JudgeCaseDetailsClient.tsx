...
import type { JudgeCaseDetails } from "@/models/judge-case.model";

export default function JudgeCaseDetailsClient({
  caseId,
}: {
  caseId: string;
}) {
  const [judgeCase, setJudgeCase] = useState<JudgeCaseDetails | null>(null);
  ...
  async function loadCaseDetails() {
    ...
    try {
      ...
      if (!res.ok) {
        setJudgeCase(null);
        setError(data.message || "Failed to load judge case details.");
      } else {
        setJudgeCase(data);
      }
    } catch {
  ...
  return (
    <div className="space-y-6 text-slate-950">
      ...
      <section className="grid gap-4 md:grid-cols-5">
        <InfoCard label="Evidence" value={String(judgeCase.evidence_count || 0)} />
        <InfoCard label="Analyzed" value={String(judgeCase.analyzed_evidence_count || 0)} />
        <InfoCard label="Lab Reports" value={String(judgeCase.lab_report_count || 0)} />
        <InfoCard label="Legal Notes" value={String(judgeCase.final_note_count || 0)} />
        <InfoCard label="Verdicts" value={String(judgeCase.verdict_count || 0)} />
      </section>

      <section className="glass-card rounded-3xl p-6">
        <h2 className="text-xl font-bold text-slate-950">Case Description</h2>
        <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-600">
          {judgeCase.description || "No description provided."}
        </p>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-3">
        ...
        <div className="glass-card rounded-3xl p-6 xl:col-span-2">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-4">
            <ReadinessCard
              title="Evidence"
              value={judgeCase.evidence_count > 0 ? "Available" : "Missing"}
              good={judgeCase.evidence_count > 0}
            />
            <ReadinessCard
              title="Lab Reports"
              value={judgeCase.lab_report_count > 0 ? "Available" : "Pending"}
              good={judgeCase.lab_report_count > 0}
            />
            <ReadinessCard
              title="Legal Notes"
              value={judgeCase.final_note_count > 0 ? "Available" : "Pending"}
              good={judgeCase.final_note_count > 0}
            />
            <ReadinessCard
              title="Verdict"
              value={judgeCase.verdict_count > 0 ? "Issued" : "Not Issued"}
              good={judgeCase.verdict_count > 0}
            />
          </div>
        </div>
      </section>
      ...
      <section className="glass-card overflow-hidden rounded-3xl">
        <div className="border-b border-slate-200 p-5">
          <h2 className="text-xl font-bold text-slate-950">Evidence</h2>
          <p className="text-sm text-slate-500">
            Blockchain evidence and file-hash proof.
          </p>
        </div>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <Panel title="Lab Reports">
          {judgeCase.labReports.length === 0 ? (
            <EmptyText text="No lab reports submitted yet." />
          ) : (
            <div className="space-y-4">
              {judgeCase.labReports.map((report) => (
        ...
        <Panel title="Legal Notes">
          {judgeCase.legalNotes.length === 0 ? (
            <EmptyText text="No legal notes submitted yet." />
          ) : (
            <div className="space-y-4">
              {judgeCase.legalNotes.map((note) => (
                <div
                  key={note.id}
      ...
      <Panel title="Verdicts">
        {judgeCase.verdicts.length === 0 ? (
          <EmptyText text="No verdict issued yet." />
        ) : (
          <div className="space-y-4">
            {judgeCase.verdicts.map((verdict) => (
              <div
                key={verdict.id}
                className="rounded-2xl border border-slate-200 bg-slate-50 p-5"
              >
                <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Decision: {verdict.decision} • Status: {verdict.status}
                    </p>
                  </div>
                ...
                <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">
                  Sentence / Order: {verdict.sentence_text || "-"}
                </p>
              </div>
            ))}
          </div>
...
function ReportCard({ report }: { report: any }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
      ...
      <p className="mt-1 text-sm text-slate-500">
        {report.analysis_type || "General Analysis"} by {report.analyzed_by}
      </p>
