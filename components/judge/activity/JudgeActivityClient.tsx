...
export default function JudgeActivityClient() {
  ...
  async function loadActivity() {
    ...
    try {
      const res = await fetch("/api/judge/activity", {
        cache: "no-store",
      });
      ...
      if (!res.ok) {
        setLogs([]);
        setError(data.message || "Failed to load judge activity logs.");
      } else {
      ...
      <section className="relative overflow-hidden rounded-3xl border border-amber-100 bg-gradient-to-br from-slate-950 via-amber-900 to-amber-500 p-7 text-white shadow-xl shadow-amber-200/70">
        <div className="relative">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-amber-50">
            Review judge actions such as creating, updating, and deleting
            verdicts.
          </p>
        </div>
      </section>
