...
import type { LegalEvidenceDetails } from "@/models/legal-evidence.model";
import EvidenceTimeline from "@/components/evidence/EvidenceTimeline";

export default function LegalEvidenceDetailsModal({
  evidence,
  onClose,
}: {
  evidence: LegalEvidenceDetails;
  onClose: () => void;
}) {
  return (
      ...
      <div className="dark-scrollbar max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="space-y-6 p-6">
          <section className="grid gap-4 md:grid-cols-4">
            ...
            <InfoCard
              label="Created At"
              value={formatDate(evidence.created_at)}
            />
          </section>
          ...
          <section className="grid gap-4 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-950 p-5 text-white">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-300">
                This transaction hash helps verify that the evidence hash was
                recorded on the blockchain.
              </p>
            </div>
          </section>

          <EvidenceTimeline evidenceId={evidence.id} tone="purple" />
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5">
            ...
            {evidence.reports.length === 0 ? (
              <p className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
                No lab report is attached to this evidence yet.
              </p>
            ) : (
