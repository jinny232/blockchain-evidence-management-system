...
import type { LegalDashboardResponse } from "@/models/legal-dashboard.model";

export default function LegalDashboardClient() {
  const [data, setData] = useState<LegalDashboardResponse | null>(null);
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      if (!res.ok) {
        setData(null);
        setError(json.message || "Failed to load legal dashboard.");
      } else {
        setData(json);
      }
    } catch {
  ...
  if (error || !data) {
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-700">
        {error || "Could not load legal dashboard."}
      </div>
    );
  }
    ...
    <div className="space-y-6 text-slate-950">
        ...
        <div className="relative flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-violet-50">
              ...
              and prepare legal opinions for court review.
            </p>
          </div>
          ...
          <div className="flex items-center justify-between border-b border-slate-200 p-5">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Latest cases assigned to legal counsel.
              </p>
            </div>
        ...
        <div className="space-y-6">
          <div className="glass-card rounded-3xl p-6">
            ...
            <div className="mt-5 space-y-3">
              <WorkflowStep title="Review Case" text="Check assigned case details and investigation summary." />
              <WorkflowStep title="Review Evidence" text="Verify evidence hash, IPFS CID, and blockchain transaction." />
              <WorkflowStep title="Review Lab Result" text="Read lab analysis and prepare legal remarks." />
            </div>
          </div>
      ...
      <section className="glass-card overflow-hidden rounded-3xl">
        <div className="flex items-center justify-between border-b border-slate-200 p-5">
          <div>
            <h2 className="text-xl font-bold text-slate-950">
              Recent Evidence for Legal Review
            </h2>
            <p className="text-sm text-slate-500">
              Evidence from cases assigned to you.
            </p>
          </div>
