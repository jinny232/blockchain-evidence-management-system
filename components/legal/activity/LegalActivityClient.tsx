...
export default function LegalActivityClient() {
  const [logs, setLogs] = useState<LegalActivityLog[]>([]);
  ...
  async function loadActivity() {
    ...
    try {
      const res = await fetch("/api/legal/activity", {
        cache: "no-store",
      });
      ...
      if (!res.ok) {
        setLogs([]);
        setError(data.message || "Failed to load legal activity logs.");
      } else {
      ...
      <section className="relative overflow-hidden rounded-3xl border border-violet-100 bg-gradient-to-br from-violet-700 via-indigo-800 to-slate-950 p-7 text-white shadow-xl shadow-violet-200/70">
        ...
        <div className="relative">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-violet-50">
            Review your legal actions such as creating, updating, and deleting
            legal notes.
          </p>
        </div>
      </section>
