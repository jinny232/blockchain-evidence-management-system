...
import { getJudgeDashboardData } from "@/services/judge-dashboard.service";
...
export async function handleGetJudgeDashboard(request: NextRequest) {
  try {
    const auth = await getJudgeIdFromRequest(request);

    if (!auth.ok) return auth.response;

    const data = await getJudgeDashboardData(auth.judgeId);
    ...
    return NextResponse.json(
      { message: "Failed to load judge dashboard." },
      { status: 500 }
    );
  }
}